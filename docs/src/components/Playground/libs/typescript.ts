...
export function tsVersionSupportsInlayHints(version: string) {
  ...
  return (
    ...
    (tsMajor &&
      tsMinor &&
      Number.parseInt(tsMajor) === 4 &&
      Number.parseInt(tsMinor) >= 6)
  );
}

export function getTSPlaygroundUrlFromSandbox(sandbox: TSSandbox) {
  return `https://www.typescriptlang.org/play?${sandbox.createURLQueryWithCompilerOptions(
    sandbox
  )}`;
}
...
function isValidTSReleases(releases: unknown): releases is TSReleases {
  return (
    ...
    Array.isArray((releases as TSReleases).versions)
  );
}
